...
# Energy-extended rules: general + commodity-relevant rules
# Oil is a commodity — commodity_breakout and dollar_weakness apply
ENERGY_RULES_12 = GENERAL_RULES_10 + ["commodity_breakout", "dollar_weakness"]

LEAN_RULES_3 = ["trend_continuation", "seasonality", "death_cross"]

# Energy baseline: lean 3 rules, moderate parameters
# Energy stocks track oil prices with varying beta
#
...
ALT_BASELINES = [
    ...
    (
        "Alt D: Energy-extended rules (12 rules, 10%/5%)",
        dict(
            rules=ENERGY_RULES_12,
            min_confidence=0.50,
            profit_target=0.10,
            stop_loss=1.0,
            max_loss_pct=5.0,
...
def generate_tune_configs(
    ...
    Energy-specific: commodity_breakout and dollar_weakness are available
    as additional rules. volume_breakout may help diversify entries.
    """
    configs = []
    ...
    if "Bootstrap" in failed_gates:
        ...
        # Energy-extended rules (general + commodity_breakout + dollar_weakness)
        configs.append((
            "BS tune: energy rules (12)",
            {**base_kwargs, "rules": ENERGY_RULES_12, "min_confidence": 0.50, "cooldown_bars": 3},
        ))
        ...
        # Add commodity_breakout (oil is a commodity)
        if "commodity_breakout" not in base_rules:
            configs.append((
                "BS tune: + commodity_breakout",
    ...
    if "Regime" in failed_gates:
        base_rules = base_kwargs.get("rules", [])
        # Add commodity_breakout (may fire in non-bull regimes — oil spikes in crisis)
        if "commodity_breakout" not in base_rules:
            configs.append((
                "Regime tune: + commodity_breakout",
                {**base_kwargs, "rules": base_rules + ["commodity_breakout"]},
            ))
        # Add dollar_weakness (USD weakens in different regimes)
        if "dollar_weakness" not in base_rules:
            configs.append((
                "Regime tune: + dollar_weakness",
                {**base_kwargs, "rules": base_rules + ["dollar_weakness"]},
            ))
        # Energy-extended rules
        configs.append((
            "Regime tune: energy rules (12)",
            {**base_kwargs, "rules": ENERGY_RULES_12, "min_confidence": 0.50, "cooldown_bars": 3},
        ))
