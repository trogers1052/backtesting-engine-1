#!/usr/bin/env python3
"""
Financial Sector Validate-Then-Tune Optimization

Generic validation script for financial stocks (banks, insurance, payments, REITs).
Validates through 4 statistical gates, diagnoses failures, and tunes.

Includes sector-specific rules:
- financial_mean_reversion: BB support + ADX < 25 + RSI oversold in golden cross
- financial_seasonality: Q1/Q4 strength, May-Sep weakness for banks

Usage:
    python validate_financials.py JPM
    python validate_financials.py GS
    python validate_financials.py XLF
    python validate_financials.py V
"""
...
SYMBOL_INFO = {
    "JPM": {
        "name": "JPMorgan Chase",
        "description": "Largest US bank — investment banking, retail, commercial, asset mgmt",
        "tier": "Large-cap diversified bank",
    },
    "GS": {
        "name": "Goldman Sachs",
        "description": "Premier investment bank — M&A, trading, wealth management, consumer",
        "tier": "Investment bank",
    },
    "BRK.B": {
        "name": "Berkshire Hathaway",
        "description": "Insurance conglomerate — GEICO, BNSF, equity portfolio (Buffett)",
        "tier": "Insurance conglomerate",
    },
    "V": {
        "name": "Visa",
        "description": "Global payment network — credit/debit card rails, fintech partnerships",
        "tier": "Payment network",
    },
    "MA": {
        "name": "Mastercard",
        "description": "Global payment network — transaction processing, data analytics",
        "tier": "Payment network",
    },
    "MCO": {
        "name": "Moody's Corporation",
        "description": "Credit ratings, research, risk analytics — duopoly with S&P Global",
        "tier": "Financial data/ratings",
    },
    "SPGI": {
        "name": "S&P Global",
        "description": "Credit ratings, market intelligence, indices — owns S&P 500 brand",
        "tier": "Financial data/ratings",
    },
    "PGR": {
        "name": "Progressive Corporation",
        "description": "Auto/home insurance — direct-to-consumer, Snapshot telematics",
        "tier": "Insurance",
    },
    "CB": {
        "name": "Chubb Limited",
        "description": "Global P&C insurance — commercial, personal, specialty lines",
        "tier": "Insurance",
    },
    "XLF": {
        "name": "Financial Select Sector SPDR ETF",
        "description": "Broad financials sector basket — BRK.B, JPM, V, MA top holdings",
        "tier": "Financials sector ETF",
    },
    "NU": {
        "name": "Nu Holdings",
        "description": "Brazilian digital bank — fintech, credit cards, lending, crypto",
        "tier": "Fintech (emerging market)",
    },
    "O": {
        "name": "Realty Income",
        "description": "Monthly dividend REIT — retail, industrial, gaming triple-net leases",
        "tier": "REIT",
    },
    "AMT": {
        "name": "American Tower",
        "description": "Cell tower REIT — wireless infrastructure, data centers, global",
        "tier": "Infrastructure REIT",
    },
}

# General rules (same as other sectors for comparability)
GENERAL_RULES_10 = [
...
# Financial-specific rules: general + financial_mean_reversion + financial_seasonality
FINANCIAL_RULES_12 = GENERAL_RULES_10 + [
    "financial_mean_reversion", "financial_seasonality",
]

# Lean baseline for financials: mean-reversion focused
# Financials are mean-reverting, so we lead with mean_reversion + seasonality + death_cross
LEAN_RULES_3 = ["trend_continuation", "seasonality", "death_cross"]

# Financial-focused lean rules: the new sector-specific rules
FINANCIAL_LEAN_3 = ["financial_mean_reversion", "financial_seasonality", "death_cross"]
...
def generate_tune_configs(
    ...
    Financial-specific: financial_mean_reversion and financial_seasonality
    are available as additional rules. volume_breakout may help diversify entries.
    """
    configs = []
    ...
    if "Bootstrap" in failed_gates:
        ...
        # Financial-extended rules (general + financial_mean_reversion + financial_seasonality)
        configs.append((
            "BS tune: financial rules (12)",
            {**base_kwargs, "rules": FINANCIAL_RULES_12, "min_confidence": 0.50, "cooldown_bars": 3},
        ))
        # Add volume_breakout for more entries
        ...
        # Add financial_mean_reversion
        if "financial_mean_reversion" not in base_rules:
            configs.append((
                "BS tune: + financial_mean_reversion",
                {**base_kwargs, "rules": base_rules + ["financial_mean_reversion"]},
            ))
    ...
    if "Regime" in failed_gates:
        base_rules = base_kwargs.get("rules", [])
        # Add financial_mean_reversion (mean-reversion works across regimes)
        if "financial_mean_reversion" not in base_rules:
            configs.append((
                "Regime tune: + financial_mean_reversion",
                {**base_kwargs, "rules": base_rules + ["financial_mean_reversion"]},
            ))
        # Add financial_seasonality
        if "financial_seasonality" not in base_rules:
            configs.append((
                "Regime tune: + financial_seasonality",
                {**base_kwargs, "rules": base_rules + ["financial_seasonality"]},
            ))
        # Financial-extended rules
        configs.append((
            "Regime tune: financial rules (12)",
            {**base_kwargs, "rules": FINANCIAL_RULES_12, "min_confidence": 0.50, "cooldown_bars": 3},
        ))
        # Full general rule set
        configs.append((
        ...
        # Tighter stops (catch bear/chop losses faster)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_financials.py <SYMBOL>")
