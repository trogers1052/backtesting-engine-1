#!/usr/bin/env python3
"""
Consumer Staples — Validate-Then-Tune Optimization

New batch: beverages, household products, mass retail, retail growth, sector ETFs.
Per-stock rule recommendations based on sub-sector research:

- Beverages (KO): ultra-defensive, Dividend King, beta 0.11-0.36, RSI 40
- Household (PG, CL, CHD, KMB): ultra-low beta (0.02-0.31), textbook mean-reversion
- Mass retail (WMT): hybrid defensive/growth, beta 0.26-0.66
- Retail growth (COST): MOMENTUM stock (beta ~1.0, P/E ~50) — exclude from mean-reversion
- Sector ETF (XLP): diversified, dampened volatility, best position sizing

Key sector insights:
- Consumer staples are among the lowest-beta stocks (even lower than utilities)
- RSI oversold thresholds must be HIGHER (40-42) — these stocks rarely dip
- ADX < 18 confirms mean-reverting regime (tighter than utilities' 20)
- SMA_200 is the absolute floor — never buy below it
- COST is NOT a typical staple — use momentum rules instead
- Tighter profit targets (6-7%) match the available range for ultra-low-vol names
- Q1 strong, summer defensive rotation, Sep universally weak, Q4 holiday strength

Usage:
    python validate_consumer_staples.py KO
    python validate_consumer_staples.py PG
    python validate_consumer_staples.py COST
    python validate_consumer_staples.py WMT
    python validate_consumer_staples.py CHD
    python validate_consumer_staples.py CL
    python validate_consumer_staples.py KMB
    python validate_consumer_staples.py XLP
"""
...
# KO has a 33-day data gap at 2022-01-10 — start after gap
...
SYMBOL_INFO = {
    "KO": {
        "name": "Coca-Cola",
        "description": "Beverages bellwether — beta 0.11-0.36, yield 2.6%, 62yr dividend streak",
        "tier": "Beverages (Dividend King)",
    },
    "PG": {
        "name": "Procter & Gamble",
        "description": "Consumer staple bellwether — beta 0.15, household products leader",
        "tier": "Household products",
    },
    "COST": {
        "name": "Costco",
        "description": "Warehouse retail — beta ~1.0, P/E ~50, membership model, MOMENTUM stock",
        "tier": "Retail growth (MOMENTUM)",
    },
    "WMT": {
        "name": "Walmart",
        "description": "Mass retail — beta 0.26-0.66, e-commerce growth hybrid",
        "tier": "Mass retail",
    },
    "CHD": {
        "name": "Church & Dwight",
        "description": "Household products — beta 0.02-0.47, Arm & Hammer, ultra-low vol",
        "tier": "Household products",
    },
    "CL": {
        "name": "Colgate-Palmolive",
        "description": "Household products — beta 0.03-0.30, negative skew, global consumer staple",
        "tier": "Household products",
    },
    "KMB": {
        "name": "Kimberly-Clark",
        "description": "Household products — beta 0.08-0.31, yield ~5%, Dividend King",
        "tier": "Household products (Dividend King)",
    },
    "XLP": {
        "name": "Consumer Staples Select Sector SPDR",
        "description": "Large-cap consumer staples ETF — benchmark, mean-reverting, diversified",
        "tier": "Consumer staples sector ETF",
    },
}
...
# Consumer staples-specific rules
STAPLES_RULES_13 = GENERAL_RULES_10 + [
    "consumer_staples_mean_reversion", "consumer_staples_pullback", "consumer_staples_seasonality",
]

# Lean sets by sub-sector character
STAPLES_LEAN_REVERSION = ["consumer_staples_mean_reversion", "consumer_staples_seasonality", "death_cross"]
STAPLES_LEAN_PULLBACK = ["consumer_staples_pullback", "consumer_staples_seasonality", "death_cross"]
STAPLES_LEAN_BALANCED = ["consumer_staples_mean_reversion", "consumer_staples_pullback", "consumer_staples_seasonality", "death_cross"]

# COST should use momentum rules, NOT staples mean reversion
COST_LEAN_MOMENTUM = ["trend_continuation", "momentum_reversal", "death_cross"]
COST_LEAN_TECH = ["tech_ema_pullback", "tech_seasonality", "death_cross"]

# Per-stock recommendations based on research
STOCK_RULE_RECOMMENDATIONS = {
    # Beverages — ultra-defensive, Dividend King
    "KO": STAPLES_LEAN_REVERSION,     # Ultra-low beta, textbook mean-reverter

    # Household products — ultra-low beta, textbook mean-reversion
    "PG": STAPLES_LEAN_REVERSION,     # Beta 0.15, consumer staple bellwether
    "CL": STAPLES_LEAN_REVERSION,     # Beta 0.03-0.30, very tight range
    "CHD": STAPLES_LEAN_REVERSION,    # Beta 0.02-0.47, ultra-low vol
    "KMB": STAPLES_LEAN_REVERSION,    # Beta 0.08-0.31, Dividend King

    # Mass retail — slightly higher beta, hybrid defensive/growth
    "WMT": STAPLES_LEAN_BALANCED,     # Beta 0.26-0.66, e-commerce growth

    # Retail growth — MOMENTUM stock, exclude from mean-reversion
    "COST": COST_LEAN_MOMENTUM,       # Beta ~1.0, P/E ~50, trades like tech

    # Sector ETF
    "XLP": STAPLES_LEAN_BALANCED,     # Diversified, mean-reverting
}

STOCK_SUBSECTOR = {
    "KO": "beverages",
    "PG": "household", "CL": "household", "CHD": "household", "KMB": "household",
    "WMT": "mass_retail",
    "COST": "retail_growth",
    "XLP": "staples_etf",
}
...
def get_alt_baselines(symbol: str) -> list:
    recommended_rules = STOCK_RULE_RECOMMENDATIONS.get(symbol, STAPLES_LEAN_BALANCED)
    sub_sector = STOCK_SUBSECTOR.get(symbol, "household")
    ...
    baselines = [
        ...
        (
            f"Alt D: Staples rules ({len(STAPLES_RULES_13)} rules, 10%/5%)",
            dict(rules=STAPLES_RULES_13, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ),
        (
    ...
    if sub_sector == "beverages":
        # Ultra-defensive, tight PT matches tiny price swings
        baselines.append((
            "Alt F: Beverages tight (6%/3%, conf=0.55, cooldown=7)",
            dict(rules=STAPLES_LEAN_REVERSION, min_confidence=0.55, profit_target=0.06,
                 stop_loss=1.0, max_loss_pct=3.0, cooldown_bars=7,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Beverages moderate (7%/4%)",
            dict(rules=STAPLES_LEAN_BALANCED, min_confidence=0.50, profit_target=0.07,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "household":
        # Ultra-low beta, very tight ranges
        baselines.append((
    ...
    elif sub_sector == "mass_retail":
        # Slightly higher beta, can tolerate wider PT
        baselines.append((
            "Alt F: Mass retail balanced (8%/4%)",
            dict(rules=STAPLES_LEAN_BALANCED, min_confidence=0.50, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Mass retail moderate (7%/4%)",
            dict(rules=STAPLES_LEAN_REVERSION, min_confidence=0.50, profit_target=0.07,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "retail_growth":
        # COST is a momentum stock — wide PT, wider stops
        baselines.append((
            "Alt F: COST momentum (15%/8%)",
            dict(rules=COST_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: COST tech rules (12%/7%)",
            dict(rules=COST_LEAN_TECH, min_confidence=0.50, profit_target=0.12,
                 stop_loss=1.0, max_loss_pct=7.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt H: COST wide (20%/10%)",
            dict(rules=COST_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.20,
                 stop_loss=1.0, max_loss_pct=10.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "staples_etf":
...
def generate_tune_configs(failed_gates, base_kwargs, symbol=""):
    ...
    recommended = STOCK_RULE_RECOMMENDATIONS.get(symbol, STAPLES_LEAN_BALANCED)
    sub_sector = STOCK_SUBSECTOR.get(symbol, "household")
    ...
    if "Walk-Forward" in failed_gates:
        ...
        if "consumer_staples_mean_reversion" not in base_rules:
            configs.append(("WF tune: + staples_mean_reversion",
                            {**base_kwargs, "rules": base_rules + ["consumer_staples_mean_reversion"]}))
        if "consumer_staples_pullback" not in base_rules:
            configs.append(("WF tune: + staples_pullback",
                            {**base_kwargs, "rules": base_rules + ["consumer_staples_pullback"]}))
    ...
    if "Bootstrap" in failed_gates:
        ...
        configs.append(("BS tune: staples rules (13)",
                        {**base_kwargs, "rules": STAPLES_RULES_13, "min_confidence": 0.50, "cooldown_bars": 3}))
    ...
    if "Regime" in failed_gates:
        if "consumer_staples_mean_reversion" not in base_rules:
            configs.append(("Regime tune: + staples_mean_reversion",
                            {**base_kwargs, "rules": base_rules + ["consumer_staples_mean_reversion"]}))
        if "consumer_staples_pullback" not in base_rules:
            configs.append(("Regime tune: + staples_pullback",
                            {**base_kwargs, "rules": base_rules + ["consumer_staples_pullback"]}))
        configs.append(("Regime tune: staples rules (13)",
                        {**base_kwargs, "rules": STAPLES_RULES_13, "min_confidence": 0.50, "cooldown_bars": 3}))
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_consumer_staples.py <SYMBOL>")
    ...
    if start_date != START_DATE:
        console.print(f"  [yellow]Using custom start date {start_date} (data gap workaround)[/yellow]")
