#!/usr/bin/env python3
"""
IAUM (iShares Gold Micro ETF) Validate-Then-Tune Optimization

Validates IAUM gold ETF rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

IAUM is Tier 2: 64.7% WR, 0.76 Sharpe, 3.96 PF, +96.3%, -11.8% DD.
iShares Gold Micro — low-cost gold exposure, tight moves, high win rate.
Current config: 10 rules, PT=5%, ML=4%, cooldown=3, conf=0.50.

Usage:
    python validate_iaum.py
"""
...
# Current rules.yaml config (10 rules, gold ETF with tight PT)
BASELINE = dict(
    rules=[
    ...
    min_confidence=0.50,
    profit_target=0.05,     # 5% — tight target for gold's predictable moves
...
ALT_BASELINES = [
    ...
    (
        "Alt C: Higher confidence (10 rules, 5%/4%, conf=0.65)",
        dict(
            rules=BASELINE["rules"][:],
            min_confidence=0.65,
            profit_target=0.05,
...
def generate_tune_configs(
    ...
    if "Walk-Forward" in failed_gates:
        # Gold is low-volatility — try tighter and wider PTs
        for pt in [0.03, 0.04, 0.06, 0.08]:
            configs.append((
                f"WF tune: PT={pt:.0%}",
                {**base_kwargs, "profit_target": pt},
            ))
        # ATR-based stops (gold has low ATR — may help)
        for atr in [2.0, 2.5, 3.0]:
            configs.append((
                f"WF tune: ATR {atr}x",
                {**base_kwargs, "stop_mode": "atr", "atr_multiplier": atr},
            ))
        # Longer cooldown (gold trends slowly)
        for cb in [5, 7]:
            configs.append((
    ...
    if "Regime" in failed_gates:
        # Add commodity_breakout (worked for PPLT)
        ...
        # Add dollar_weakness (gold inversely correlates with USD)
        ...
        # Drop exit-oriented rules that may not diversify regime
        configs.append((
...
def _write_config_section(f, report: ValidationReport):
    ...
    f.write(f"- **Profit Target:** {kw.get('profit_target', 0.05):.0%}\n")
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "iaum-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# IAUM (iShares Gold Micro ETF) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Tier 2 (64.7% WR, 0.76 Sharpe, 3.96 PF)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL/UUUU/PPLT/SLV validations:\n\n")
        ...
        f.write("IAUM is a Tier 2 gold ETF — tight 5% profit target, 10 rules, high win rate.\n\n")
        ...
        if best_report and best_report.all_passed:
            f.write(f"**IAUM validated.** The {best_report.label} config passes all 4 validation gates.\n\n")
            ...
            f.write(f"    profit_target: {kw.get('profit_target', 0.05)}\n")
            ...
            f.write(
                f"**IAUM shows limited edge.** "
                f"Best config: {best_report.label} ({best_report.pass_count}/4 gates).\n\n"
            )
            _write_config_section(f, best_report)
            f.write("### Deployment Recommendation\n\n")
            f.write("- Consider downgrading from Tier 2 to Tier 3\n")
            f.write("- Only trade during validated regime conditions\n")
            f.write("- Reduce position sizing to half\n\n")

        else:
            f.write(
                f"**IAUM should be downgraded or removed.** "
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]IAUM (iShares Gold Micro ETF) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Current status: Tier 2 (64.7% WR, 0.76 Sharpe, 3.96 PF)\n"
    ...
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append((
            f"rules.yaml baseline (10 rules, 5%/4%)",
    ...
    if not all_candidates:
        ...
        baseline_report = ValidationReport(
            label="rules.yaml baseline (10 rules, 5%/4%)", kwargs=BASELINE,
        )
