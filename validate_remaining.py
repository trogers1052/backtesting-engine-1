#!/usr/bin/env python3
"""
Remaining Unvalidated Symbols — Validate-Then-Tune Optimization

12 symbols across multiple sectors, each mapped to the best-fit existing rules.

Per-stock research and rule assignments:

- B (Barnes Group): Aerospace components, industrial. Beta ~1.3, mid-cap.
  → Industrial rules (mean-reversion + pullback). Similar to ETN/CAT cyclicals.

- CNQ (Canadian Natural Resources): Major Canadian oil & gas producer, beta ~1.1.
  → Energy rules. Heavy oil sands + conventional. Similar to COP/EOG.

- EWY (iShares MSCI South Korea ETF): Samsung/SK Hynix heavy (~40% tech).
  → Tech rules. Behaves like a high-beta tech proxy with EM overlay.

- MELI (MercadoLibre): Latin American e-commerce + fintech. Beta ~1.5, high-growth.
  → Tech growth rules (EMA pullback + momentum). Similar to AMZN behavior.

- OKLO (Oklo Inc): Nuclear micro-reactors. IPO May 2024, pre-revenue, speculative.
  → Mining/uranium rules. Trades with URA/URNM sentiment. Very short history.

- S (SentinelOne): Cloud cybersecurity. Beta ~1.4, not yet profitable.
  → Tech rules. Similar to CRWD/PANW/NET behavior pattern.

- SDGR (Schrödinger): Computational drug discovery platform. Beta ~1.8.
  → Tech rules (computational platform), with healthcare seasonality crossover.

- SNDK (Sandisk): Storage/memory, spun off from WD Oct 2024. Short history.
  → Tech/semiconductor rules. Similar to MU/AMAT.

- TEM (Tempus AI): AI-powered genomics/diagnostics. IPO Jun 2024, pre-profit.
  → Tech rules. Healthcare-adjacent but trades like AI/growth stock.

- USAR (USA Rare Earth): Rare earth mining/processing. Micro-cap, speculative.
  → Mining rules. Similar to MP/UUUU/REMX rare earth plays.

- WLDN (Willdan Group): Engineering/infrastructure consulting. Small-cap industrial.
  → Industrial rules. Infrastructure services, government contracts.

- XLY (Consumer Discretionary Select Sector SPDR): Sector ETF, AMZN/TSLA heavy.
  → Generic lean rules + tech seasonality. Behaves like a growth-weighted ETF.

Usage:
    python validate_remaining.py B
    python validate_remaining.py CNQ
    python validate_remaining.py MELI
    python validate_remaining.py ALL   # run all sequentially
"""
...
# Recent IPOs/spinoffs — use later start dates since pre-IPO data doesn't exist
SYMBOL_START_OVERRIDES = {
    "B": date(2025, 9, 1),       # Ticker recycled: was Barnes Group (acquired May 2025) → now Barrick Mining
    "OKLO": date(2024, 5, 15),   # IPO via SPAC merger May 2024
    "SNDK": date(2024, 10, 1),   # Spun off from Western Digital Oct 2024
    "TEM": date(2024, 6, 15),    # IPO June 2024
}

SYMBOL_INFO = {
    "B": {
        "name": "Barrick Mining",
        "description": "Gold/copper mining major — beta ~0.7, largest gold miner, dividend payer",
        "tier": "Mining / Gold (major producer)",
    },
    "CNQ": {
        "name": "Canadian Natural Resources",
        "description": "Major Canadian oil & gas — beta ~1.1, heavy oil sands + conventional, dividend grower",
        "tier": "Energy (integrated oil & gas)",
    },
    "EWY": {
        "name": "iShares MSCI South Korea ETF",
        "description": "South Korea country ETF — ~40% Samsung/SK Hynix, tech-heavy, beta ~1.1",
        "tier": "International ETF (tech-heavy)",
    },
    "MELI": {
        "name": "MercadoLibre",
        "description": "Latin American e-commerce + fintech — beta ~1.5, high-growth, AMZN of LatAm",
        "tier": "Consumer Discretionary / Tech (high-growth)",
    },
    "OKLO": {
        "name": "Oklo Inc",
        "description": "Nuclear micro-reactors — pre-revenue, speculative, trades with uranium sentiment",
        "tier": "Energy / Nuclear (SPECULATIVE — limited history)",
    },
    "S": {
        "name": "SentinelOne",
        "description": "Cloud cybersecurity — beta ~1.4, AI-powered endpoint security, not yet profitable",
        "tier": "Tech / Cybersecurity (growth)",
    },
    "SDGR": {
        "name": "Schrödinger",
        "description": "Computational drug discovery — beta ~1.8, physics-based molecular simulation",
        "tier": "Tech / Biotech (computational platform)",
    },
    "SNDK": {
        "name": "Sandisk",
        "description": "Flash storage/memory — spun from WD Oct 2024, semiconductor adjacent",
        "tier": "Tech / Semiconductors (SPINOFF — limited history)",
    },
    "TEM": {
        "name": "Tempus AI",
        "description": "AI genomics/diagnostics — IPO Jun 2024, pre-profit, healthcare AI",
        "tier": "Tech / Healthcare AI (SPECULATIVE — limited history)",
    },
    "USAR": {
        "name": "USA Rare Earth",
        "description": "Rare earth mining/processing — micro-cap, speculative, critical minerals play",
        "tier": "Mining / Rare Earth (speculative)",
    },
    "WLDN": {
        "name": "Willdan Group",
        "description": "Engineering/infrastructure consulting — small-cap, government contracts, clean energy",
        "tier": "Industrial (infrastructure services)",
    },
    "XLY": {
        "name": "Consumer Discretionary Select Sector SPDR",
        "description": "Sector ETF — AMZN ~22%, TSLA ~14%, growth-weighted consumer discretionary",
        "tier": "Consumer Discretionary ETF",
    },
}

# ============================================================================
# Rule Sets — using existing sector rules, matched per-stock
# ============================================================================
...
IND_RULES_13 = GENERAL_RULES_10 + ["industrial_mean_reversion", "industrial_pullback", "industrial_seasonality"]
...
ENERGY_LEAN_BALANCED = ["energy_momentum", "energy_mean_reversion", "energy_seasonality", "death_cross"]
ENERGY_RULES_14 = GENERAL_RULES_10 + ["energy_momentum", "energy_mean_reversion", "energy_seasonality", "midstream_yield_reversion"]
...
TECH_RULES_13 = GENERAL_RULES_10 + ["tech_ema_pullback", "tech_mean_reversion", "tech_seasonality"]

# Mining sector
MINING_LEAN = ["commodity_breakout", "seasonality", "death_cross"]
MINING_VOLUME = ["volume_breakout", "seasonality", "death_cross"]
MINING_BALANCED = ["commodity_breakout", "volume_breakout", "miner_metal_ratio", "seasonality", "death_cross"]

# Per-stock rule recommendations based on research
STOCK_RULE_RECOMMENDATIONS = {
    # Mining / Gold
    "B": MINING_BALANCED,          # Barrick Mining, gold/copper major
    "WLDN": IND_LEAN_PULLBACK,     # Infrastructure consulting, trending

    # Energy
    "CNQ": ENERGY_LEAN_BALANCED,   # Integrated oil & gas, dividend grower
    "OKLO": TECH_LEAN_MOMENTUM,    # Speculative nuclear, trades like growth stock

    # Tech / Growth
    "EWY": TECH_LEAN_BALANCED,     # Korea ETF, 40% semiconductors
    "MELI": TECH_LEAN_MOMENTUM,    # High-growth e-commerce, momentum stock
    "S": TECH_LEAN_MOMENTUM,       # Cybersecurity growth, not yet profitable
    "SDGR": TECH_LEAN_BALANCED,    # Computational biotech platform
    "SNDK": TECH_LEAN_BALANCED,    # Memory/storage, semiconductor adjacent
    "TEM": TECH_LEAN_MOMENTUM,     # AI healthcare, speculative growth

    # Mining
    "USAR": MINING_BALANCED,       # Rare earth mining, similar to MP/UUUU

    # Consumer Discretionary ETF
    "XLY": LEAN_RULES_3,           # Sector ETF, generic lean rules
}

STOCK_SECTOR = {
    "B": "mining_gold", "WLDN": "industrial",
    "CNQ": "energy",
    "OKLO": "nuclear_speculative",
    "EWY": "international_tech", "MELI": "tech_growth",
    "S": "tech_cyber", "SDGR": "tech_biotech",
    "SNDK": "tech_semi", "TEM": "tech_healthcare_ai",
    "USAR": "mining_rare_earth",
    "XLY": "consumer_disc_etf",
}
...
def get_alt_baselines(symbol: str) -> list:
    recommended_rules = STOCK_RULE_RECOMMENDATIONS.get(symbol, LEAN_RULES_3)
    sector = STOCK_SECTOR.get(symbol, "general")
    ...
    baselines = [
        ...
        (
            f"Alt D: Recommended rules ({len(recommended_rules)} rules, 10%/5%)",
    ...
    if sector == "mining_gold":
        baselines.append((
            "Alt E: Mining full (5 rules, 10%/5%)",
            dict(rules=MINING_BALANCED, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: Mining momentum (12%/6%)",
            dict(rules=["commodity_breakout", "volume_breakout", "death_cross"],
                 min_confidence=0.50, profit_target=0.12,
                 stop_loss=1.0, max_loss_pct=6.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Gold miner mean-reversion (8%/4%)",
            dict(rules=["miner_metal_ratio", "seasonality", "death_cross"],
                 min_confidence=0.55, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sector == "industrial":
        baselines.append((
            f"Alt E: Industrial full ({len(IND_RULES_13)} rules, 10%/5%)",
            dict(rules=IND_RULES_13, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: Industrial reversion (8%/4%)",
            dict(rules=IND_LEAN_REVERSION, min_confidence=0.55, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Industrial momentum (12%/6%)",
            dict(rules=["trend_continuation", "momentum_reversal", "death_cross"],
                 min_confidence=0.50, profit_target=0.12,
                 stop_loss=1.0, max_loss_pct=6.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        ...
        baselines.append((
            f"Alt E: Energy full ({len(ENERGY_RULES_14)} rules, 10%/5%)",
            dict(rules=ENERGY_RULES_14, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: Energy momentum (12%/6%)",
            dict(rules=ENERGY_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.12,
                 stop_loss=1.0, max_loss_pct=6.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Energy reversion (8%/4%)",
            dict(rules=ENERGY_LEAN_REVERSION, min_confidence=0.55, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sector in ("tech_growth", "tech_cyber", "tech_semi", "tech_healthcare_ai", "international_tech"):
        baselines.append((
            f"Alt E: Tech full ({len(TECH_RULES_13)} rules, 10%/5%)",
            dict(rules=TECH_RULES_13, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: Tech momentum (15%/7%)",
            dict(rules=TECH_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=7.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Tech reversion (8%/4%)",
            dict(rules=TECH_LEAN_REVERSION, min_confidence=0.55, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        if sector == "tech_growth":
            # MELI: wider targets for high-growth
            baselines.append((
                "Alt H: Growth wide (20%/8%)",
                dict(rules=TECH_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.20,
                     stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                     timeframe="daily", exit_timeframe="daily"),
            ))
    elif sector == "tech_biotech":
        baselines.append((
            f"Alt E: Tech full ({len(TECH_RULES_13)} rules, 10%/5%)",
            dict(rules=TECH_RULES_13, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
        ...
        # SDGR: hybrid tech + healthcare
        baselines.append((
            "Alt F: Tech + healthcare seasonality (10%/5%)",
            dict(rules=["tech_ema_pullback", "tech_mean_reversion", "healthcare_seasonality", "death_cross"],
                 min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Wide momentum (15%/8%)",
            dict(rules=TECH_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sector == "nuclear_speculative":
        # OKLO: speculative nuclear, limited history, trades like uranium miners
        baselines.append((
            "Alt E: Mining/uranium rules (10%/5%)",
            dict(rules=MINING_BALANCED, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: Wide momentum (20%/10%)",
            dict(rules=TECH_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.20,
                 stop_loss=1.0, max_loss_pct=10.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Mining volume breakout (15%/8%)",
            dict(rules=MINING_VOLUME, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sector == "mining_rare_earth":
        baselines.append((
            "Alt E: Mining full (5 rules, 10%/5%)",
            dict(rules=MINING_BALANCED, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
        ...
        baselines.append((
            "Alt F: Mining momentum (15%/8%)",
            dict(rules=["commodity_breakout", "volume_breakout", "death_cross"],
                 min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Mining wide (20%/10%)",
            dict(rules=MINING_BALANCED, min_confidence=0.50, profit_target=0.20,
                 stop_loss=1.0, max_loss_pct=10.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sector == "consumer_disc_etf":
        baselines.append((
            "Alt E: Tech rules (ETF is growth-weighted)",
            dict(rules=TECH_LEAN_BALANCED, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt F: ETF tight (6%/3%, conf=0.55)",
            dict(rules=LEAN_RULES_3, min_confidence=0.55, profit_target=0.06,
                 stop_loss=1.0, max_loss_pct=3.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: ETF moderate (8%/4%)",
            dict(rules=LEAN_RULES_3, min_confidence=0.50, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
...
def run_full_validation(runner, symbol, label, run_kwargs, start_date=START_DATE, print_reports=True):
...
def generate_tune_configs(failed_gates, base_kwargs, symbol=""):
    ...
    recommended = STOCK_RULE_RECOMMENDATIONS.get(symbol, LEAN_RULES_3)
    sector = STOCK_SECTOR.get(symbol, "general")
    ...
    if "Walk-Forward" in failed_gates:
        ...
        if recommended != base_rules:
            configs.append((f"WF tune: recommended rules",
                            {**base_kwargs, "rules": recommended}))
    ...
    if "Bootstrap" in failed_gates:
        ...
        if recommended != base_rules:
            configs.append((f"BS tune: recommended rules",
    ...
    if "Regime" in failed_gates:
        if recommended != base_rules:
            configs.append(("Regime tune: recommended rules",
                            {**base_kwargs, "rules": recommended}))
...
def write_markdown_report(symbol, info, baseline_report, alt_reports, tune_reports, best_report, total_elapsed, start_date):
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_remaining.py <SYMBOL|ALL>")
        print(f"  Symbols: {', '.join(SYMBOL_INFO.keys())}")
        print(f"  ALL = run all symbols sequentially")
        sys.exit(1)

    arg = sys.argv[1].upper()

    if arg == "ALL":
        symbols_to_run = list(SYMBOL_INFO.keys())
    ...
    for symbol in symbols_to_run:
        ...
        if start_date != START_DATE:
            console.print(f"  [yellow]Using start override: {start_date} (limited history)[/yellow]")
        ...
        try:
            ...
            console.print(f"  [red]No data for {symbol}: {e}[/red]")
            console.print(f"  [yellow]Skipping {symbol}[/yellow]\n")
            continue
        ...
        best_report = run_full_validation(runner, symbol, best_screen.label, best_screen.kwargs,
                                          start_date=start_date, print_reports=True)
        ...
        if best_report.failed_gates:
            ...
            for label, kwargs, _ in top_n:
                report = run_full_validation(runner, symbol, label, kwargs,
                                              start_date=start_date, print_reports=True)
        ...
        if winner.pass_count >= 2:
            ...
            mtf_report = run_full_validation(runner, symbol, f"{winner.label} [multi-TF]", mtf_kwargs,
                                              start_date=start_date, print_reports=True)
        ...
        write_markdown_report(symbol, info, baseline_report, alt_reports, tune_reports, best_report, total_elapsed, start_date)
        ...
        console.print("")  # blank line between symbols
