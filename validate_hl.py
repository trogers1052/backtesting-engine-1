#!/usr/bin/env python3
"""
HL (Hecla Mining) Validate-Then-Tune Optimization

Validates HL mining rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

HL is a silver miner, previously blacklisted (36.8% WR, -0.07 Sharpe).
This script either confirms the blacklist or finds a validated config.

Usage:
    python validate_hl.py
"""
...
# HL baseline: mining-focused rules for silver miner
# Prior best result was broad ruleset at 12%/6% = +153.9%, 0.49 Sharpe
# Start with focused mining rules (similar to CCJ approach)
BASELINE = dict(
    rules=[
        "trend_continuation",    # Core trend following
        "seasonality",           # Silver seasonal patterns (Q1, Q3)
        "death_cross",           # Downside protection
        "commodity_breakout",    # Mining-specific breakout
        "miner_metal_ratio",     # Silver-specific mean reversion
    ],
    min_confidence=0.65,
    profit_target=0.12,     # 12% PT (silver is more volatile than uranium)
...
# Alternative baselines to screen alongside
ALT_BASELINES = [
    (
    ...
    (
        "Alt B: Broad mining (8 rules, 12%/6%)",
        dict(
            rules=[
                "trend_continuation", "seasonality", "death_cross",
                "commodity_breakout", "miner_metal_ratio",
                "dollar_weakness", "volume_breakout", "momentum_reversal",
            ],
            min_confidence=0.65,
            profit_target=0.12,
            stop_loss=1.0,
    ...
    (
        "Alt D: Conservative (3 rules, 8%/4%)",
        dict(
            rules=["trend_continuation", "seasonality", "death_cross"],
            min_confidence=0.70,
            profit_target=0.08,
...
def generate_tune_configs(
    ...
    if "Bootstrap" in failed_gates:
        # Not enough statistical edge — need more trades or better edge
    ...
    if "Regime" in failed_gates:
        ...
        configs.append((
            ...
            {**base_kwargs, "rules": base_kwargs["rules"] + ["dollar_weakness"]},
        ))
        configs.append((
...
def write_markdown_report(
    ...
    """Write hl-validated.md with full validation evidence."""
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "hl-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# HL (Hecla Mining) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Blacklisted (36.8% WR, -0.07 Sharpe, -38.7% DD)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("### Validate-Then-Tune Approach\n\n")
        f.write("Same methodology as CCJ validation:\n\n")
        f.write("1. **Screen Baselines** — Test multiple starting configs to find the best baseline\n")
        ...
        f.write("HL was previously blacklisted, so multiple baseline configs were screened:\n\n")
        ...
        if tune_reports:
            ...
            # Full validation details of tuned configs
        ...
        if best_report and best_report.all_passed:
            f.write(f"**HL can be un-blacklisted.** The {best_report.label} config passes all 4 validation gates:\n\n")
            ...
            cmd_parts = [
                ...
                f"--profit-target {kw.get('profit_target', 0.12)}",
            ...
            if kw.get("stop_mode") == "atr":
                ...
                cmd_parts.append(f"--atr-multiplier {kw.get('atr_multiplier', 2.0)}")
            cmd_parts.append("--walk-forward --bootstrap --monte-carlo --regime-analysis")
            f.write(f"### Reproduce with CLI\n\n```\n{' '.join(cmd_parts)}\n```\n\n")

        elif best_report and best_report.pass_count >= 3:
            f.write(
                f"**HL shows potential but does not fully validate.** "
                f"Best config: {best_report.label} ({best_report.pass_count}/4 gates).\n\n"
            )
            f.write("Consider:\n")
            f.write("- Trading with reduced position sizing (half normal size)\n")
            f.write("- Only trading during validated regime conditions\n")
            f.write("- Re-validating after 6 more months of data\n\n")

        else:
            f.write(
                "**HL blacklist confirmed.** No configuration passes enough validation gates "
                "to justify live trading.\n\n"
            )
            best_passes = best_report.pass_count if best_report else 0
            f.write(f"Best result: {best_passes}/4 gates passed.\n\n")
            f.write("### Why HL Fails Validation\n\n")
            f.write("HL (Hecla Mining) is a silver miner with characteristics that make it difficult to trade profitably:\n\n")
            f.write("1. **Silver is more volatile than uranium/gold** — larger drawdowns, noisier signals\n")
            f.write("2. **Seasonal patterns are weaker** — silver procurement doesn't have the same institutional demand cycle as uranium\n")
            f.write("3. **Small-cap mining stock** — more susceptible to idiosyncratic risk (mine closures, labor disputes, etc.)\n")
            f.write("4. **No validated edge exists** in our rule set for this symbol\n\n")
            f.write("### Alternative Silver/Mining Exposure\n\n")
            f.write("Better-performing alternatives in the mining/metals space:\n\n")
            f.write("| Symbol | Description | Historical WR | Sharpe |\n")
            f.write("|--------|-------------|---------------|--------|\n")
            f.write("| WPM | Wheaton Precious Metals | 48.6% | 0.67 |\n")
            f.write("| SLV | Silver ETF | 56.9% | 0.60 |\n")
            f.write("| CCJ | Cameco (Uranium) | 57.6% | 0.83 |\n\n")
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]HL (Hecla Mining) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Prior status: BLACKLISTED (36.8% WR, -0.07 Sharpe)\n"
    ...
    # Pick the best baseline by Sharpe (min 5 trades)
    all_candidates = []
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append(("Primary baseline (5 mining rules, 12%/6%)", BASELINE, baseline_bt))
    ...
    if not all_candidates:
        console.print("\n[red]No baseline produced enough trades. HL is untradeable.[/red]")
        # Write minimal report
        baseline_report = ValidationReport(
            label="Primary baseline (5 mining rules, 12%/6%)",
            kwargs=BASELINE,
        )
    ...
    # Create alt reports for the markdown
    alt_reports = []
    for alt_label, alt_kwargs, alt_bt in alt_results:
    ...
    # ── Step 2: Full validation of best baseline ───────────────────────────
    console.print(f"\n[bold blue]STEP 2: Full Validation of Best Baseline[/bold blue]")
    validated_report = run_full_validation(
        runner, best_label, best_kwargs
    )

    if validated_report.all_passed:
        console.print(
            "\n[bold green]Best baseline passes all 4 validation gates! "
    ...
    # ── Step 4: Quick screen tuning configs ────────────────────────────────
    console.print(
        f"\n[bold blue]STEP 4: Quick Screen "
    ...
    elapsed = time.time() - total_start
    write_markdown_report(
        baseline_report, alt_reports, tune_reports,
        best_overall if best_overall.all_passed else best_overall,
        elapsed,
    )
