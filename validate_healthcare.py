#!/usr/bin/env python3
"""
Healthcare / Biotech — Validate-Then-Tune Optimization

New batch: pharma, diversified, managed care, biotech, devices, sector ETF.
Per-stock rule recommendations based on sub-sector research:

- Large-cap pharma (ABBV): mean-reverting, beta 0.32-0.36, RSI 38-40
- Diversified healthcare (JNJ): ultra-defensive, beta 0.14-0.35, RSI 40-42
- Managed care (UNH): mean-reverting w/ wider stops, beta 0.42, RSI 30
- Large-cap biotech (VRTX): hybrid low-beta biotech, beta 0.23-0.49, RSI 35
- Medical devices (SYK): growth compounder, momentum rules, beta 0.69-0.87
- Med-tech growth (TMDX): high-growth, momentum only, beta 1.1-2.05
- Clinical-stage biotech (RXRX, VKTX): EXCLUDED — binary trial risk
- Sector ETF (XLV): mean-reverting, diversified, RSI 35-38

Key sector insights:
- Seasonality: Nov-Dec strongest (75% win rate), Sep weakest (-1.2%)
- Q1 pharma weakness from insurance deductible resets
- RXRX (beta 3.21) and VKTX (Phase 3 obesity trials) are pure speculation
- SYK and TMDX are growth/momentum — exclude from mean-reversion
- JNJ behaves like a consumer staple (ultra-low beta, Dividend King)

Usage:
    python validate_healthcare.py ABBV
    python validate_healthcare.py JNJ
    python validate_healthcare.py RXRX
    python validate_healthcare.py SYK
    python validate_healthcare.py TMDX
    python validate_healthcare.py UNH
    python validate_healthcare.py VKTX
    python validate_healthcare.py VRTX
    python validate_healthcare.py XLV
"""
...
SYMBOL_INFO = {
    "ABBV": {
        "name": "AbbVie",
        "description": "Large-cap pharma — beta 0.32-0.36, yield ~3%, 12yr dividend growth, Humira franchise",
        "tier": "Large-cap pharma (mean-reverting)",
    },
    "JNJ": {
        "name": "Johnson & Johnson",
        "description": "Diversified healthcare — beta 0.14-0.35, 54yr Dividend King, post-Kenvue spinoff",
        "tier": "Diversified healthcare (ultra-defensive)",
    },
    "RXRX": {
        "name": "Recursion Pharmaceuticals",
        "description": "AI clinical-stage biotech — beta 3.21, pre-revenue, binary trial risk",
        "tier": "Clinical-stage biotech (EXCLUDED — binary risk)",
    },
    "SYK": {
        "name": "Stryker",
        "description": "Medical devices — beta 0.69-0.87, growth compounder, 11% organic sales growth",
        "tier": "Medical devices (MOMENTUM)",
    },
    "TMDX": {
        "name": "TransMedics",
        "description": "Organ transplant tech — beta 1.1-2.05, high-growth (+37% YoY), ROE 54%",
        "tier": "Med-tech growth (MOMENTUM)",
    },
    "UNH": {
        "name": "UnitedHealth Group",
        "description": "Managed care — beta 0.42, 3rd largest in S&P 500 Healthcare, political gap risk",
        "tier": "Managed care (mean-reverting w/ gap risk)",
    },
    "VKTX": {
        "name": "Viking Therapeutics",
        "description": "Clinical biotech — Phase 3 obesity/GLP-1 trials, 50-70% gap risk on readouts",
        "tier": "Clinical-stage biotech (EXCLUDED — binary risk)",
    },
    "VRTX": {
        "name": "Vertex Pharmaceuticals",
        "description": "Large-cap biotech — beta 0.23-0.49, $12B CF revenue, unusually low-beta for biotech",
        "tier": "Large-cap biotech (hybrid)",
    },
    "XLV": {
        "name": "Health Care Select Sector SPDR",
        "description": "Large-cap healthcare ETF — beta 0.75, diversified across pharma/devices/managed care",
        "tier": "Healthcare sector ETF",
    },
}
...
HEALTHCARE_RULES_13 = GENERAL_RULES_10 + [
    "healthcare_mean_reversion", "healthcare_pullback", "healthcare_seasonality",
]

# Lean sets by sub-sector character
HC_LEAN_REVERSION = ["healthcare_mean_reversion", "healthcare_seasonality", "death_cross"]
HC_LEAN_PULLBACK = ["healthcare_pullback", "healthcare_seasonality", "death_cross"]
HC_LEAN_BALANCED = ["healthcare_mean_reversion", "healthcare_pullback", "healthcare_seasonality", "death_cross"]

# Momentum stocks (SYK, TMDX) — NOT healthcare mean reversion
HC_LEAN_MOMENTUM = ["trend_continuation", "momentum_reversal", "death_cross"]
HC_LEAN_TECH = ["tech_ema_pullback", "tech_seasonality", "death_cross"]

# Clinical biotech (RXRX, VKTX) — excluded from sector rules, use general only
HC_CLINICAL_GENERAL = ["enhanced_buy_dip", "rsi_oversold", "death_cross"]

# Per-stock recommendations based on research
STOCK_RULE_RECOMMENDATIONS = {
    # Mean-reverting (low beta, defensive)
    "ABBV": HC_LEAN_REVERSION,    # Beta 0.32, pharma bellwether
    "JNJ": HC_LEAN_REVERSION,     # Beta 0.14, ultra-defensive like PG
    "UNH": HC_LEAN_BALANCED,      # Beta 0.42, mean-reverts w/ wider stops
    "VRTX": HC_LEAN_BALANCED,     # Beta 0.23-0.49, hybrid low-beta biotech
    "XLV": HC_LEAN_BALANCED,      # Beta 0.75, ETF diversification

    # Momentum/growth (higher beta, trending)
    "SYK": HC_LEAN_MOMENTUM,      # Beta 0.69-0.87, growth compounder like COST
    "TMDX": HC_LEAN_MOMENTUM,     # Beta 1.1-2.05, high-growth

    # Clinical biotech — excluded from sector rules, general only
    "RXRX": HC_CLINICAL_GENERAL,  # Beta 3.21, binary trial risk
    "VKTX": HC_CLINICAL_GENERAL,  # Phase 3 obesity trials
}

STOCK_SUBSECTOR = {
    "ABBV": "pharma", "JNJ": "diversified",
    "UNH": "managed_care", "VRTX": "large_biotech",
    "SYK": "med_devices", "TMDX": "med_growth",
    "RXRX": "clinical_biotech", "VKTX": "clinical_biotech",
    "XLV": "healthcare_etf",
}
...
def get_alt_baselines(symbol: str) -> list:
    recommended_rules = STOCK_RULE_RECOMMENDATIONS.get(symbol, HC_LEAN_BALANCED)
    sub_sector = STOCK_SUBSECTOR.get(symbol, "pharma")
    ...
    baselines = [
        ...
        (
            f"Alt D: Healthcare rules ({len(HEALTHCARE_RULES_13)} rules, 10%/5%)",
            dict(rules=HEALTHCARE_RULES_13, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ),
        (
    ...
    if sub_sector == "pharma":
        baselines.append((
            "Alt F: Pharma tight (7%/4%, conf=0.55, cooldown=7)",
            dict(rules=HC_LEAN_REVERSION, min_confidence=0.55, profit_target=0.07,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=7,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Pharma moderate (8%/4%)",
            dict(rules=HC_LEAN_BALANCED, min_confidence=0.50, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "diversified":
        baselines.append((
            "Alt F: Diversified tight (6%/3%, conf=0.55)",
            dict(rules=HC_LEAN_REVERSION, min_confidence=0.55, profit_target=0.06,
                 stop_loss=1.0, max_loss_pct=3.0, cooldown_bars=7,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Diversified moderate (8%/4%)",
            dict(rules=HC_LEAN_BALANCED, min_confidence=0.50, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "managed_care":
        # UNH: wider stops for political gap risk
        baselines.append((
            "Alt F: Managed care balanced (10%/6%)",
            dict(rules=HC_LEAN_BALANCED, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=6.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Managed care + financial rules (10%/7%)",
            dict(rules=["healthcare_mean_reversion", "financial_mean_reversion",
                         "healthcare_seasonality", "death_cross"],
                 min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=7.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "large_biotech":
        # VRTX: hybrid approach — can use both mean-reversion and momentum
        baselines.append((
            "Alt F: Biotech hybrid (10%/5%)",
            dict(rules=["healthcare_mean_reversion", "trend_continuation",
                         "healthcare_seasonality", "death_cross"],
                 min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ...
        baselines.append((
            "Alt G: Biotech moderate (8%/5%)",
            dict(rules=HC_LEAN_BALANCED, min_confidence=0.50, profit_target=0.08,
                 stop_loss=1.0, max_loss_pct=5.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector in ("med_devices", "med_growth"):
        # Momentum stocks — wide PT, wider stops
        baselines.append((
            "Alt F: Momentum (15%/8%)",
            dict(rules=HC_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Tech-style (12%/7%)",
            dict(rules=HC_LEAN_TECH, min_confidence=0.50, profit_target=0.12,
                 stop_loss=1.0, max_loss_pct=7.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        if sub_sector == "med_growth":
            baselines.append((
                "Alt H: Wide momentum (20%/10%)",
                dict(rules=HC_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.20,
                     stop_loss=1.0, max_loss_pct=10.0, cooldown_bars=5,
                     timeframe="daily", exit_timeframe="daily"),
            ))
    elif sub_sector == "clinical_biotech":
        # Binary risk — general rules only, try various params
        baselines.append((
            "Alt F: Clinical tight stops (10%/4%)",
            dict(rules=HC_CLINICAL_GENERAL, min_confidence=0.50, profit_target=0.10,
                 stop_loss=1.0, max_loss_pct=4.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt G: Clinical wide (20%/10%)",
            dict(rules=HC_CLINICAL_GENERAL, min_confidence=0.50, profit_target=0.20,
                 stop_loss=1.0, max_loss_pct=10.0, cooldown_bars=5,
                 timeframe="daily", exit_timeframe="daily"),
        ))
        baselines.append((
            "Alt H: Clinical momentum (15%/8%)",
            dict(rules=HC_LEAN_MOMENTUM, min_confidence=0.50, profit_target=0.15,
                 stop_loss=1.0, max_loss_pct=8.0, cooldown_bars=3,
                 timeframe="daily", exit_timeframe="daily"),
        ))
    elif sub_sector == "healthcare_etf":
        baselines.append((
...
def generate_tune_configs(failed_gates, base_kwargs, symbol=""):
    ...
    recommended = STOCK_RULE_RECOMMENDATIONS.get(symbol, HC_LEAN_BALANCED)
    sub_sector = STOCK_SUBSECTOR.get(symbol, "pharma")
    ...
    if "Walk-Forward" in failed_gates:
        ...
        if "healthcare_mean_reversion" not in base_rules:
            configs.append(("WF tune: + hc_mean_reversion",
                            {**base_kwargs, "rules": base_rules + ["healthcare_mean_reversion"]}))
        if "healthcare_pullback" not in base_rules:
            configs.append(("WF tune: + hc_pullback",
                            {**base_kwargs, "rules": base_rules + ["healthcare_pullback"]}))
    ...
    if "Bootstrap" in failed_gates:
        ...
        configs.append(("BS tune: healthcare rules (13)",
                        {**base_kwargs, "rules": HEALTHCARE_RULES_13, "min_confidence": 0.50, "cooldown_bars": 3}))
    ...
    if "Regime" in failed_gates:
        if "healthcare_mean_reversion" not in base_rules:
            configs.append(("Regime tune: + hc_mean_reversion",
                            {**base_kwargs, "rules": base_rules + ["healthcare_mean_reversion"]}))
        if "healthcare_pullback" not in base_rules:
            configs.append(("Regime tune: + hc_pullback",
                            {**base_kwargs, "rules": base_rules + ["healthcare_pullback"]}))
        configs.append(("Regime tune: healthcare rules (13)",
                        {**base_kwargs, "rules": HEALTHCARE_RULES_13, "min_confidence": 0.50, "cooldown_bars": 3}))
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_healthcare.py <SYMBOL>")
