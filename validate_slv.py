#!/usr/bin/env python3
"""
SLV (Silver ETF) Validate-Then-Tune Optimization

Validates SLV silver ETF rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

SLV is Tier 2: 56.9% WR, 0.60 Sharpe, 3.05 PF, +139.5%, -17.9% DD.
iShares Silver Trust — industrial/precious metal hybrid, high volatility.
Current config: 14 rules, PT=12%, ML=4%, cooldown=7, conf=0.50.

Usage:
    python validate_slv.py
"""
...
# Current rules.yaml config (14 rules, silver-specific including commodity rules)
BASELINE = dict(
    rules=[
        "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
        "rsi_oversold", "rsi_overbought", "macd_bearish_crossover",
        "trend_alignment", "trend_break_warning", "commodity_breakout",
        "miner_metal_ratio", "dollar_weakness", "seasonality",
        "volume_breakout", "death_cross",
    ],
    min_confidence=0.50,
    profit_target=0.12,     # 12% — wider target for silver volatility
...
ALT_BASELINES = [
    (
        "Alt A: Core 7 rules (PPLT-style, 6%/4%)",
        dict(
            rules=[
                "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
    ...
    (
        "Alt C: Higher confidence (14 rules, 12%/4%, conf=0.65)",
        dict(
            rules=BASELINE["rules"][:],
            min_confidence=0.65,
            profit_target=0.12,
...
def generate_tune_configs(
    ...
    if "Walk-Forward" in failed_gates:
        # Try different profit targets (silver is volatile — may need wider or tighter)
        for pt in [0.08, 0.10, 0.15, 0.20]:
            configs.append((
                f"WF tune: PT={pt:.0%}",
                {**base_kwargs, "profit_target": pt},
            ))
        # ATR-based stops (adapt to silver volatility)
    ...
    if "Regime" in failed_gates:
        # SLV already has commodity_breakout, dollar_weakness, seasonality
        # Try removing or adjusting rules to improve regime diversity
        configs.append((
            "Regime tune: fewer rules (drop rsi_overbought + macd_bearish)",
            {**base_kwargs, "rules": [
                r for r in base_kwargs["rules"]
                if r not in ("rsi_overbought", "macd_bearish_crossover")
            ]},
        ))
        configs.append((
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "slv-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# SLV (Silver ETF) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Tier 2 (56.9% WR, 0.60 Sharpe, 3.05 PF)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL/UUUU/PPLT validations:\n\n")
        ...
        f.write("SLV is a Tier 2 performer — 14 rules including silver-specific commodity rules.\n\n")
        ...
        if best_report and best_report.all_passed:
            f.write(f"**SLV validated.** The {best_report.label} config passes all 4 validation gates.\n\n")
            ...
            f.write(
                f"**SLV should be downgraded or removed.** "
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]SLV (Silver ETF) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Current status: Tier 2 (56.9% WR, 0.60 Sharpe, 3.05 PF)\n"
    ...
    console.print(f"\n  Screening primary baseline (rules.yaml 14-rule config)...")
    try:
    ...
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append((
            f"rules.yaml baseline (14 rules, 12%/4%)",
    ...
    if not all_candidates:
        ...
        baseline_report = ValidationReport(
            label="rules.yaml baseline (14 rules, 12%/4%)", kwargs=BASELINE,
        )
