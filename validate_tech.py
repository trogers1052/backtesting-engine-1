#!/usr/bin/env python3
"""
Tech Sector Validate-Then-Tune Optimization

Generic validation script for technology stocks.
Validates through 4 statistical gates, diagnoses failures, and tunes.

Usage:
    python validate_tech.py SOFI
    python validate_tech.py AMD
    python validate_tech.py AAPL
    python validate_tech.py NET
    python validate_tech.py PANW
    python validate_tech.py XLK
"""
...
SYMBOL_INFO = {
    "SOFI": {
        ...
        "description": "Fintech — digital banking, lending, investing platform",
        "tier": "Mid-cap fintech",
    },
    "AMD": {
        "name": "Advanced Micro Devices",
        "description": "Semiconductor — CPUs, GPUs, data center AI chips",
        "tier": "Large-cap semiconductor",
    },
    "AAPL": {
        "name": "Apple",
        "description": "Mega-cap consumer tech — iPhone, Mac, Services",
        "tier": "Mega-cap tech",
    },
    "NET": {
        "name": "Cloudflare",
        "description": "Cloud infrastructure — CDN, security, edge computing",
        "tier": "Mid-cap cloud/security",
    },
    "PANW": {
        "name": "Palo Alto Networks",
        "description": "Cybersecurity — firewalls, cloud security, SASE",
        "tier": "Large-cap cybersecurity",
    },
    "XLK": {
        "name": "Technology Select Sector SPDR ETF",
        "description": "Broad tech sector basket — AAPL, MSFT, NVDA top holdings",
        "tier": "Tech sector ETF",
    },
}

# General rules (no commodity rules for tech stocks)
GENERAL_RULES_10 = [
...
# Tech baseline: lean 3 rules, moderate parameters
# Tech stocks are higher beta than defense/energy, more momentum-driven
#
# SPEED OPTIMIZATION: Use daily-only for screening & tuning,
# then multi-TF (daily entries + 5min exits) only for final validation.
...
def run_full_validation(
    runner: BacktraderRunner, symbol: str, label: str,
    run_kwargs: Dict, print_reports: bool = True,
) -> ValidationReport:
    ...
    try:
        ...
        wf_result = validator.validate_simple(
            symbol, START_DATE, END_DATE, train_pct=0.7, embargo_days=5, purge_days=10,
            **run_kwargs,
        )
        elapsed = time.time() - t0
    ...
    try:
        ...
        bs_detail = (
            f"p={bs_result.p_value:.4f}, Sharpe CI=[{bs_result.sharpe_ci_lower:.2f}, "
    ...
    try:
        ...
        mc_detail = (
            f"Ruin={mc_result.ruin_probability:.1%}, P95 DD=-{mc_result.drawdown_p95:.1f}%, "
...
def generate_tune_configs(failed_gates: List[str], base_kwargs: Dict) -> List[tuple]:
    """Generate targeted tuning configs. Tech-specific: no commodity rules,
    but volume_breakout may help for momentum-driven tech stocks."""
    configs = []
...
def _write_config_section(f, report: ValidationReport):
    ...
    if report.backtest:
        b = report.backtest
        f.write(
            f"**Performance:** Return={b.total_return:+.1%}, Trades={b.total_trades}, "
...
def write_markdown_report(symbol, info, baseline_report, alt_reports, tune_reports, best_report, total_elapsed):
    filepath = os.path.join(os.path.expanduser("~/Projects/backtesting-service"), f"{symbol.lower()}-validated.md")
    with open(filepath, "w") as f:
        ...
        for name, r in all_screens:
            if r.backtest:
                b = r.backtest
                f.write(f"| {name} | {b.total_trades} | {b.win_rate:.1%} | {b.total_return:+.1%} | {b.sharpe_ratio or 0:.2f} | {b.profit_factor or 0:.2f} | -{b.max_drawdown_pct or 0:.1f}% |\n")
            else:
                f.write(f"| {name} | - | - | - | - | - | - |\n")
        ...
        for i, r in enumerate(all_validated):
            ...
            b = r.backtest
            if b:
                f.write(f"| {bold}{r.label}{bold} | {wf} | {bs} | {mc} | {rg} | {bold}{b.sharpe_ratio or 0:.2f}{bold} | {bold}{b.total_return:+.1%}{bold} | {b.total_trades} |\n")
        f.write("\n---\n\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_tech.py <SYMBOL>")
    ...
    console.print(
        f"  Baseline: {bl_result.total_trades} trades, WR={bl_result.win_rate:.1%}, "
    ...
    baseline_report = ValidationReport(label="Lean 3 rules baseline (10%/5%, conf=0.50)", kwargs=BASELINE.copy(), backtest=bl_result)
    ...
    for alt_name, alt_kwargs in ALT_BASELINES:
        ...
        try:
            ...
            console.print(
                f"  {alt_name}: {alt_result.total_trades} trades, WR={alt_result.win_rate:.1%}, "
    ...
    best_screen = max(valid_screens, key=lambda r: r.backtest.sharpe_ratio or 0) if valid_screens else baseline_report
    console.print(f"\n  [bold green]Best by Sharpe: {best_screen.label} (Sharpe={best_screen.backtest.sharpe_ratio or 0:.2f})[/bold green]")
    ...
    if best_report.failed_gates:
        ...
        tune_configs = generate_tune_configs(best_report.failed_gates, best_screen.kwargs)
        ...
        for label, kwargs, result in top_n:
            console.print(f"    {label}: Sharpe={result.sharpe_ratio or 0:.2f}, Return={result.total_return:+.1%}, WR={result.win_rate:.1%}")
    ...
    if winner.pass_count >= 2:
        ...
        if mtf_report.backtest and mtf_report.backtest.total_trades >= 2:
            ...
            console.print(f"\n  Multi-TF result: {mtf_report.pass_count}/4 gates, Sharpe={mtf_report.backtest.sharpe_ratio or 0:.2f}")
        else:
