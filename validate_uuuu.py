#!/usr/bin/env python3
"""
UUUU (Energy Fuels) Validate-Then-Tune Optimization

Validates UUUU uranium mining rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

UUUU is active Tier 3 (high risk): 48.6% WR, 0.50 Sharpe, -33.9% DD.
Two competing configs: 9-rule rules.yaml vs 3-rule prior best.

Usage:
    python validate_uuuu.py
"""
...
# Current rules.yaml config (9 rules, aggressive)
BASELINE = dict(
    rules=[
    ...
    min_confidence=0.50,
    profit_target=0.20,     # 20% — let winners run (high-beta)
    stop_loss=1.0,          # Disabled — max_loss_pct is the effective stop
    max_loss_pct=4.0,       # Tight 4% stop due to volatility
    cooldown_bars=3,
    timeframe="daily",
    exit_timeframe="5min",
)
...
ALT_BASELINES = [
    (
        "Alt A: Prior best (3 core rules, 10%/5%)",
        dict(
    ...
    (
        "Alt C: Core + exits (6 rules, 10%/5%)",
        dict(
            rules=[
                "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
                "trend_alignment", "trend_break_warning", "death_cross",
            ],
            min_confidence=0.50,
            profit_target=0.10,
            stop_loss=1.0,
    ...
    (
        "Alt D: Core + mining (7 rules, 12%/5%)",
        dict(
            rules=[
                "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
                "commodity_breakout", "seasonality", "death_cross",
                "miner_metal_ratio",
            ],
            min_confidence=0.55,
            profit_target=0.12,
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "uuuu-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# UUUU (Energy Fuels) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Active Tier 3 (48.6% WR, 0.50 Sharpe, -33.9% DD)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL validations:\n\n")
        ...
        f.write("UUUU has two competing configs in the codebase:\n")
        f.write("- **rules.yaml (current):** 9 rules, PT=20%, ML=4%, cooldown=3 (aggressive, high-beta)\n")
        f.write("- **Prior best:** 3 core rules, PT=10%, ML=5%, cooldown=5 (55.2% WR, +269.9%)\n\n")
        ...
        if best_report and best_report.all_passed:
            f.write(f"**UUUU validated.** The {best_report.label} config passes all 4 validation gates.\n\n")
            ...
            cmd_parts = [
                ...
                f"--min-confidence {kw.get('min_confidence', 0.50)}",
                f"--max-loss {kw.get('max_loss_pct', 5.0)}",
                f"--cooldown-bars {kw.get('cooldown_bars', 5)}",
            ...
            f.write(
                f"**UUUU partially validates.** "
                f"Best config: {best_report.label} ({best_report.pass_count}/4 gates).\n\n"
            )
            _write_config_section(f, best_report)
            f.write("### Deployment Recommendation\n\n")
            f.write("- Keep as Tier 3 (high risk) with reduced position sizing\n")
            f.write("- Deploy with regime-aware sizing (reduce in non-bull markets)\n")
            f.write("- Re-validate after 6 months of additional data\n\n")

        elif best_report and best_report.pass_count >= 2:
            f.write(
                f"**UUUU shows limited edge.** "
                f"Best config: {best_report.label} ({best_report.pass_count}/4 gates).\n\n"
            )
            _write_config_section(f, best_report)
            f.write("### Deployment Recommendation\n\n")
            f.write("- Consider downgrading from Tier 3 active to watchlist-only\n")
            f.write("- Only trade during validated regime conditions\n")
            f.write("- Half position sizing compared to current Tier 3 allocation\n\n")

        else:
            f.write(
                f"**UUUU should be blacklisted.** "
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]UUUU (Energy Fuels) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Current status: Active Tier 3 (48.6% WR, 0.50 Sharpe)\n"
    ...
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append((
            f"rules.yaml baseline (9 rules, 20%/4%)",
    ...
    if not all_candidates:
        ...
        baseline_report = ValidationReport(
            label="rules.yaml baseline (9 rules, 20%/4%)", kwargs=BASELINE,
        )
