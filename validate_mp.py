#!/usr/bin/env python3
"""
MP (MP Materials) Validate-Then-Tune Optimization

Validates MP rare earth miner rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

MP is Tier 3: 63.9% WR, +299.9%, 0.47 Sharpe, 3.21 PF, -26.3% DD.
MP Materials — only US rare earth miner. High WR, huge return, but low Sharpe
suggests volatile equity curve. Only 3 rules (CCJ-style lean config), PT=12%, ML=5%.

Usage:
    python validate_mp.py
"""
...
# Current rules.yaml config (3 rules, rare earth miner — lean CCJ-style)
BASELINE = dict(
    ...
    max_loss_pct=5.0,       # 5% max loss (wider than most — rare earths are volatile)
    cooldown_bars=5,
    timeframe="daily",
    exit_timeframe="5min",
)
...
ALT_BASELINES = [
    (
        "Alt A: Full rule set (10 rules, 12%/5%)",
        dict(
            rules=[
    ...
    (
        "Alt B: Full rules + tighter stop (10 rules, 12%/4%)",
        dict(
            rules=[
                "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
                "rsi_oversold", "macd_bearish_crossover", "trend_alignment",
    ...
    (
        "Alt D: Higher confidence (3 rules, 12%/5%, conf=0.65)",
        dict(
            rules=["trend_continuation", "seasonality", "death_cross"],
            min_confidence=0.65,
            profit_target=0.12,
...
def generate_tune_configs(
    ...
    MP-specific: Rare earth miner, only US source. Very volatile, cyclical.
    Currently lean 3-rule config — main lever is expanding rule set and PT tuning.
    """
    configs = []

    if "Walk-Forward" in failed_gates:
        # Rare earths are volatile — try different PT levels
        ...
        # ATR-based stops (rare earths gap hard)
        ...
        for ml in [3.0, 4.0]:
            configs.append((
                f"WF tune: max_loss={ml}%",
                {**base_kwargs, "max_loss_pct": ml},
            ))

    if "Bootstrap" in failed_gates:
        # More rules = more trades = more statistical power
        full_rules = [
            "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
            "rsi_oversold", "macd_bearish_crossover", "trend_alignment",
            "golden_cross", "trend_break_warning", "death_cross", "seasonality",
        ...
        configs.append((
            ...
            {**base_kwargs, "rules": full_rules},
        ))
        configs.append((
            "BS tune: full rules + tighter stop 4%",
            {**base_kwargs, "rules": full_rules, "max_loss_pct": 4.0, "cooldown_bars": 3},
        ))
    ...
    if "Regime" in failed_gates:
        ...
        # Add dollar_weakness (rare earths less USD-correlated than gold, but worth testing)
        if "dollar_weakness" not in base_rules:
            configs.append((
                "Regime tune: + dollar_weakness",
                {**base_kwargs, "rules": base_rules + ["dollar_weakness"]},
            ))
        # Full rule set (more rules = potentially more regime coverage)
        full_rules = [
            "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
            "rsi_oversold", "macd_bearish_crossover", "trend_alignment",
            "golden_cross", "trend_break_warning", "death_cross", "seasonality",
        ...
        # Full rules + tighter parameters
        configs.append((
            "Regime tune: full rules + tighter (4%/3cb)",
            {**base_kwargs, "rules": full_rules, "max_loss_pct": 4.0, "cooldown_bars": 3},
        ))
        # Volume breakout (worked well for CAT)
        ...
        configs.append((
            "Regime tune: higher confidence=0.65",
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "mp-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# MP (MP Materials) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Tier 3 (63.9% WR, 0.47 Sharpe, 3.21 PF)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL/UUUU/PPLT/SLV/IAUM/CAT/WPM validations:\n\n")
        ...
        f.write(
            "MP is a Tier 3 rare earth miner — only US source. High WR (63.9%) but "
            "low Sharpe (0.47) suggests volatile equity curve. 3 rules (CCJ-style lean), "
            "12% PT, 5% ML.\n\n"
        )

        # Screening table
        all_screens = [(f"rules.yaml baseline (3 rules, 12%/5%)", baseline_report)] + [
        ...
        # Sort by Sharpe
        all_validated.sort(
            key=lambda r: r.backtest.sharpe_ratio or 0 if r.backtest else 0,
            reverse=True,
        )
        ...
        f.write(
            f"**MP {'fully validates' if final.all_passed else 'partially validates'}.** "
        ...
        if final.pass_count >= 3:
            f.write("- Keep as Tier 1 with no restrictions\n")
...
def main():
    ...
    console.print(Panel.fit(
        "[bold cyan]MP Materials — Validate-Then-Tune[/bold cyan]\n"
        "Rare earth miner, only US source\n"
        "Prior: 63.9% WR, +299.9%, 0.47 Sharpe, 3.21 PF",
        border_style="cyan",
    ))
    ...
    # Run baseline
    console.print(f"\n  Running baseline: {SYMBOL} (3 rules, 12%/5%)...")
    t0 = time.time()
    ...
    baseline_report = ValidationReport(
        label="rules.yaml baseline (3 rules, 12%/5%)",
