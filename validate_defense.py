#!/usr/bin/env python3
"""
Defense Sector Validate-Then-Tune Optimization

Generic validation script for defense/aerospace stocks.
Validates through 4 statistical gates, diagnoses failures, and tunes.

Usage:
    python validate_defense.py LMT
    python validate_defense.py RTX
    python validate_defense.py AVAV
    python validate_defense.py ITA
"""
...
SYMBOL_INFO = {
    "LMT": {
        ...
        "description": "Defense prime contractor — F-35, missiles, space",
        "tier": "Large-cap defense",
    },
    "RTX": {
        "name": "RTX Corp (Raytheon)",
        "description": "Defense/aerospace — Pratt & Whitney, missiles, radar",
        "tier": "Large-cap defense",
    },
    "AVAV": {
        "name": "AeroVironment",
        "description": "Small drones, loitering munitions (Switchblade), defense tech",
        "tier": "Mid-cap defense tech",
    },
    "ITA": {
        "name": "iShares US Aerospace & Defense ETF",
        "description": "Broad defense/aerospace basket ETF",
        "tier": "Defense ETF",
    },
}

# General rules (no commodity-specific rules for defense stocks)
GENERAL_RULES_10 = [
...
# Defense baseline: lean 3 rules, moderate parameters
# Defense stocks are steadier than commodity miners, less volatile
#
...
# the best config. This avoids 30+ minute runs per backtest on liquid stocks.
...
def generate_tune_configs(
    ...
    Defense-specific: No commodity rules. Levers are PT width, confidence,
    cooldown, stop tightness, and rule set composition.
    """
    configs = []
    ...
    if "Regime" in failed_gates:
        base_rules = base_kwargs.get("rules", [])
        # Add volume_breakout (may fire in non-bull regimes)
        if "volume_breakout" not in base_rules:
            configs.append((
...
def write_markdown_report(
    ...
    with open(filepath, "w") as f:
        ...
        f.write("1. **Screen Baselines** — Test lean 3-rule + full 10-rule + alternatives\n")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python validate_defense.py <SYMBOL>")
        print("  Symbols: LMT, RTX, AVAV, ITA")
        sys.exit(1)
