#!/usr/bin/env python3
"""
WPM (Wheaton Precious Metals) Validate-Then-Tune Optimization

Validates WPM precious metals streamer rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

WPM is Tier 1: 48.6% WR, +201.2%, 0.67 Sharpe, 2.47 PF.
Wheaton Precious Metals — streaming/royalty model, leveraged to gold+silver prices.
Sub-50% WR but massive winners compensate. 10 rules, PT=12%, ML=4%, cooldown=3.

Usage:
    python validate_wpm.py
"""
...
# Current rules.yaml config (10 rules, precious metals streamer)
BASELINE = dict(
    rules=[
    ...
    min_confidence=0.50,
    profit_target=0.12,     # 12% — let precious metals trends run
...
ALT_BASELINES = [
    (
        "Alt A: PPLT-style + commodity_breakout (8 rules, 6%/4%)",
        dict(
            rules=[
                "enhanced_buy_dip", "momentum_reversal", "trend_continuation",
                "trend_alignment", "golden_cross", "trend_break_warning",
                "death_cross", "commodity_breakout",
            ],
            min_confidence=0.65,
            profit_target=0.06,
            stop_loss=1.0,
    ...
    (
        "Alt D: Higher confidence (10 rules, 12%/4%, conf=0.65)",
        dict(
            rules=BASELINE["rules"][:],
            min_confidence=0.65,
            profit_target=0.12,
...
def generate_tune_configs(
    ...
    WPM-specific: Precious metals streamer, leveraged to gold+silver.
    Sub-50% WR with big winners — PT width and commodity rules are key levers.
    """
    configs = []

    if "Walk-Forward" in failed_gates:
        # WPM trends with gold — try different PT levels
        ...
        # ATR-based stops (precious metals can be volatile)
    ...
    if "Bootstrap" in failed_gates:
        for mc in [0.40, 0.45, 0.55, 0.60, 0.65]:
            configs.append((
    ...
    if "Regime" in failed_gates:
        # Add commodity_breakout (worked for PPLT — same precious metals space)
        if "commodity_breakout" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + commodity_breakout",
                {**base_kwargs, "rules": base_kwargs["rules"] + ["commodity_breakout"]},
            ))
        # Add dollar_weakness (precious metals inversely correlate with USD)
        if "dollar_weakness" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + dollar_weakness",
                {**base_kwargs, "rules": base_kwargs["rules"] + ["dollar_weakness"]},
            ))
        # Add both commodity rules
        new_rules = base_kwargs["rules"][:]
        added = []
        if "commodity_breakout" not in new_rules:
            new_rules.append("commodity_breakout")
            added.append("commodity_breakout")
        if "dollar_weakness" not in new_rules:
            new_rules.append("dollar_weakness")
            added.append("dollar_weakness")
        if added:
            configs.append((
                f"Regime tune: + {' + '.join(added)}",
                {**base_kwargs, "rules": new_rules},
            ))
        # Add miner_metal_ratio (WPM is a streamer — ratio may help)
        if "miner_metal_ratio" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + miner_metal_ratio",
                {**base_kwargs, "rules": base_kwargs["rules"] + ["miner_metal_ratio"]},
            ))
        # Drop exit-oriented rules
        configs.append((
            "Regime tune: drop macd_bearish + rsi_oversold",
            {**base_kwargs, "rules": [
                r for r in base_kwargs["rules"]
                if r not in ("macd_bearish_crossover", "rsi_oversold")
            ]},
        ))
        # Higher confidence
        configs.append((
            "Regime tune: higher confidence=0.55",
            {**base_kwargs, "min_confidence": 0.55},
        ))
        configs.append((
            "Regime tune: higher confidence=0.60",
            {**base_kwargs, "min_confidence": 0.60},
        ))
        # PPLT-style: commodity_breakout + higher confidence
        if "commodity_breakout" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + commodity_breakout + conf=0.65",
                {**base_kwargs,
                 "rules": base_kwargs["rules"] + ["commodity_breakout"],
                 "min_confidence": 0.65},
            ))
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "wpm-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# WPM (Wheaton Precious Metals) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Tier 1 (48.6% WR, 0.67 Sharpe, 2.47 PF)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL/UUUU/PPLT/SLV/IAUM/CAT validations:\n\n")
        ...
        f.write("WPM is a Tier 1 precious metals streamer — sub-50% WR but massive winners (+201% return). 10 rules, 12% PT.\n\n")
        ...
        if best_report and best_report.all_passed:
            f.write(f"**WPM validated.** The {best_report.label} config passes all 4 validation gates.\n\n")
            ...
            f.write(f"    profit_target: {kw.get('profit_target', 0.12)}\n")
            f.write(f"    stop_loss: 0.04\n")
            f.write(f"    max_loss_pct: {kw.get('max_loss_pct', 4.0)}\n")
            f.write(f"    cooldown_bars: {kw.get('cooldown_bars', 3)}\n")
            f.write(f"  min_confidence: {kw.get('min_confidence', 0.50)}\n")
            f.write("```\n\n")

        elif best_report and best_report.pass_count >= 3:
            f.write(
                f"**WPM partially validates.** "
                f"Best config: {best_report.label} ({best_report.pass_count}/4 gates).\n\n"
            )
            _write_config_section(f, best_report)
            f.write("### Deployment Recommendation\n\n")
            f.write("- Keep as Tier 1 with validation caveats noted\n")
            ...
            f.write("- Reduce position sizing to half\n\n")

        else:
            f.write(
                f"**WPM should be downgraded or removed.** "
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]WPM (Wheaton Precious Metals) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Current status: Tier 1 (48.6% WR, 0.67 Sharpe, 2.47 PF)\n"
    ...
    console.print(f"\n  Screening primary baseline (rules.yaml 10-rule config)...")
    try:
    ...
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append((
            f"rules.yaml baseline (10 rules, 12%/4%)",
    ...
    if not all_candidates:
        ...
        baseline_report = ValidationReport(
            label="rules.yaml baseline (10 rules, 12%/4%)", kwargs=BASELINE,
        )
