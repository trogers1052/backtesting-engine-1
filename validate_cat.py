#!/usr/bin/env python3
"""
CAT (Caterpillar) Validate-Then-Tune Optimization

Validates CAT industrial stock rules through 4 statistical gates,
diagnoses any failures, and makes targeted parameter adjustments.

CAT is Tier 1: 56.5% WR, +151%, 1.54 Sharpe, 2.88 PF.
Caterpillar — industrial cyclical, the only non-mining stock in the portfolio.
Current config: 9 rules, PT=12%, ML=4%, cooldown=3, conf=0.50.

Usage:
    python validate_cat.py
"""
...
# Current rules.yaml config (9 rules, industrial cyclical)
BASELINE = dict(
    rules=[
    ...
    min_confidence=0.50,
    profit_target=0.12,     # 12% — wider target for industrial trends
...
ALT_BASELINES = [
    ...
    (
        "Alt C: Higher confidence (9 rules, 12%/4%, conf=0.65)",
        dict(
            rules=BASELINE["rules"][:],
            min_confidence=0.65,
            profit_target=0.12,
...
def generate_tune_configs(
    ...
    CAT-specific: Industrial cyclical, not a miner.
    Key levers: PT width, cooldown length, confidence, volume_breakout.
    """
    configs = []

    if "Walk-Forward" in failed_gates:
        # CAT is a strong industrial trend — try different PT levels
        ...
        # ATR-based stops (industrial stocks have moderate ATR)
        for atr in [2.0, 2.5, 3.0]:
            configs.append((
                f"WF tune: ATR {atr}x",
                {**base_kwargs, "stop_mode": "atr", "atr_multiplier": atr},
            ))
        # Longer cooldown (industrial trends are slow and steady)
        for cb in [5, 7]:
            configs.append((
    ...
    if "Regime" in failed_gates:
        # Add volume_breakout (industrials respond to volume surges)
        if "volume_breakout" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + volume_breakout",
                {**base_kwargs, "rules": base_kwargs["rules"] + ["volume_breakout"]},
            ))
        # Add commodity_breakout (CAT correlates with commodities via infrastructure)
        ...
        # Minimal rule set (trend-only)
        configs.append((
            "Regime tune: trend-only (3 rules)",
            {**base_kwargs, "rules": [
                "trend_continuation", "trend_alignment", "seasonality",
            ], "min_confidence": 0.65},
        ))
        # Higher confidence
        configs.append((
        ...
        # Combined: volume_breakout + wider PT
        if "volume_breakout" not in base_kwargs.get("rules", []):
            configs.append((
                "Regime tune: + volume_breakout + PT=15%",
                {**base_kwargs,
                 "rules": base_kwargs["rules"] + ["volume_breakout"],
                 "profit_target": 0.15},
            ))
...
def write_markdown_report(
    ...
    filepath = os.path.join(
        os.path.expanduser("~/Projects/backtesting-service"), "cat-validated.md"
    )

    with open(filepath, "w") as f:
        f.write("# CAT (Caterpillar) Validated Optimization Results\n\n")
        ...
        f.write(f"**Prior Status:** Tier 1 (56.5% WR, 1.54 Sharpe, 2.88 PF)\n\n")
        f.write("---\n\n")

        # Methodology
        f.write("## Methodology\n\n")
        f.write("Same validate-then-tune approach as CCJ/HL/UUUU/PPLT/SLV/IAUM validations:\n\n")
        ...
        f.write("CAT is Tier 1, the only industrial stock in the portfolio. 9 rules, 12% PT, the highest Sharpe (1.54).\n\n")
        ...
        if best_report and best_report.all_passed:
            f.write(f"**CAT validated.** The {best_report.label} config passes all 4 validation gates.\n\n")
            ...
            f.write(
                f"**CAT should be downgraded or removed.** "
...
def main():
    ...
    console.print(
        Panel.fit(
            "[bold blue]CAT (Caterpillar) Validate-Then-Tune Optimization[/bold blue]\n"
            f"Symbol: {SYMBOL} | Period: {START_DATE} to {END_DATE}\n"
            f"Current status: Tier 1 (56.5% WR, 1.54 Sharpe, 2.88 PF)\n"
    ...
    console.print(f"\n  Screening primary baseline (rules.yaml 9-rule config)...")
    try:
    ...
    if baseline_bt and baseline_bt.total_trades >= 5:
        all_candidates.append((
            f"rules.yaml baseline (9 rules, 12%/4%)",
    ...
    if not all_candidates:
        ...
        baseline_report = ValidationReport(
            label="rules.yaml baseline (9 rules, 12%/4%)", kwargs=BASELINE,
        )
